        ...
        if missing_mand:
            ...
            if not st.session_state.profile.get("name","").strip():
                ...
                if st.button("🌿 Calculate My Prakriti", type="primary", use_container_width=True):
                    # --- 1. Calculate Scores Before Saving ---
                    ...
                    try:
                        ...
                        if not db_result.get("db_saved"):
                            print(f"Background MySQL Save Failed: {db_result.get('db_error')}")

                    except Exception as e:
                        print(f"Background Save Error: {e}")

                    # --- 3. Transition to Results Page ---
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # ── Results sub-tabs ──────────────────────────────────────────────────────
    # res_tab3 | #"📥 Research Data Export",
    res_tab1, res_tab2, res_tab4 = st.tabs([
        "📊 Assessment Results",
    ...
    # with res_tab3:
        ...
        # if st.button("💾 Save Assessment to Database", type="primary"):
            # st.info("Step 1: Starting save process...")
            # try:
                # prof_export = dict(prof)
                # prof_export["bmi_category"] = bmi_category(prof.get("bmi", 22))
                
                # # 1. Save locally to CSV
                # st.info("Step 2: Saving to CSV...")
                # append_research_row(
                    # profile   = prof_export,
                    # responses = st.session_state.responses,
                    # trait_pct = trait_pct,
                ...
                # st.info("Step 3: Connecting to MySQL database (if it hangs here, your XAMPP database is unreachable)...")
                # import eprakruti_db as db
                ...
                # # Verify DB Save Status
                # st.info("Step 4: Checking database response...")
                # if db_result.get("db_saved"):
                    # msg = f"✅ Data successfully saved to MySQL database! (Seeker ID: {db_result.get('seeker_id')})"
                    # st.success(msg)
                    # st.toast(msg, icon="✅") # Safe popup notification
                    # st.balloons()
                # else:
                    # st.warning(f"⚠️ Saved to CSV, but MySQL database save failed: {db_result.get('db_error')}")

            # except Exception as e:
                # st.error(f"Step 5 Error: {e}")    
